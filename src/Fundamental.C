...
FundamentalEstimator::FundamentalEstimator(double precisionHz)
    : m_precision(precisionHz), m_ampFloor(DefaultAmpFloor),
      m_ampRange(DefaultAmpRange), m_freqCeiling(DefaultFreqCeiling) {
...
double FundamentalEstimator::ampFloor(void) const { return m_ampFloor; }
...
//
double FundamentalEstimator::ampRange(void) const { return m_ampRange; }
...
//
double FundamentalEstimator::freqCeiling(void) const { return m_freqCeiling; }
...
//
double FundamentalEstimator::precision(void) const { return m_precision; }
...
void FundamentalEstimator::setAmpFloor(double x) {
...
//!	\param x is the new value of this parameter.
//
void FundamentalEstimator::setAmpRange(double x) {
...
//!	\param x is the new value of this parameter.
//
void FundamentalEstimator::setFreqCeiling(double x) { m_freqCeiling = x; }
...
//!	\param x is the new value of this parameter.
//
void FundamentalEstimator::setPrecision(double x) {
...
FundamentalFromSamples::FundamentalFromSamples(double winWidthHz,
                                               double precisionHz)
    : m_cacheSampleRate(0), m_windowWidth(winWidthHz),
...
LinearEnvelope FundamentalFromSamples::buildEnvelope(
    const double *sampsBeg, const double *sampsEnd, double sampleRate,
    double tbeg, double tend, double interval, double lowerFreqBound,
    double upperFreqBound, double confidenceThreshold) {
  //  sanity check
  if (tbeg > tend) {
    std::swap(tbeg, tend);
  }
...
FundamentalFromSamples::value_type FundamentalFromSamples::estimateAt(
    const double *sampsBeg, const double *sampsEnd, double sampleRate,
    double time, double lowerFreqBound, double upperFreqBound) {
  std::vector<double> amplitudes, frequencies;
...
//! analysis.
double FundamentalFromSamples::windowWidth(void) const { return m_windowWidth; }
...
//
void FundamentalFromSamples::buildSpectrumAnalyzer(double srate) {
...
void FundamentalFromSamples::collectFreqsAndAmps(
    const double *samps, unsigned long nsamps, double sampleRate,
    std::vector<double> &frequencies, std::vector<double> &amplitudes,
    double time) {
  amplitudes.clear();
  frequencies.clear();
  ...
  if (winMiddle < sampsEnd) {
    ...
    if (!peaks.empty()) {
      ...
      Peaks::iterator maxpos = std::max_element(peaks.begin(), peaks.end(),
                                                sort_peaks_greater_amplitude);
...
LinearEnvelope FundamentalFromPartials::buildEnvelope(
...
void FundamentalFromPartials::collectFreqsAndAmps(
    PartialList::const_iterator begin_partials,
  ...
  if (begin_partials != end_partials) {
    ...
    double max_amp = 0;
    for (PartialList::const_iterator it = begin_partials; it != end_partials;
         ++it) {
