...
struct InstrumentCk {
  ...
  struct Loop {
    Int_16
        playMode; /* 0 - no loop, 1 - forward looping, 2 - backward looping */
...
struct SosEnvelopesCk {
  ...
  #define initPhaseLth ( 4*LargestLabel + 8 )
          Int_32	initPhase[initPhaseLth]; // obsolete initial phase
  array; is VARIABLE LENGTH array
                                                          // this is big enough
  for a max of 512 enhanced partials plus values below
  //	Int_32	resolution;		// frame duration in microseconds
