...
Channelizer::Channelizer(const Envelope &refChanFreq, int refChanLabel,
    ...
    : _refChannelFreq(refChanFreq.clone()), _refChannelLabel(refChanLabel),
      _stretchFactor(stretchFactor), _ampWeighting(0) {
  if (refChanLabel <= 0) {
...
Channelizer::Channelizer(double refFreq, double stretchFactor)
    : _refChannelFreq(new LinearEnvelope(refFreq)), _refChannelLabel(1),
      _stretchFactor(stretchFactor), _ampWeighting(0) {
  if (refFreq <= 0) {
...
Channelizer::Channelizer(const Channelizer &other)
    : _refChannelFreq(other._refChannelFreq->clone()),
      ...
      _stretchFactor(other._stretchFactor), _ampWeighting(other._ampWeighting) {
}
...
//!	This Channelizer is made to represent the same set of frequency
//! channels, 	constructed from the same reference Envelope and channel number
//! as
//!@a rhs.
//!
//!	\param rhs is the Channelizer to copy
//
Channelizer &Channelizer::operator=(const Channelizer &rhs) {
  if (&rhs != this) {
...
//
double Channelizer::amplitudeWeighting(void) const { return _ampWeighting; }
...
//! the Partial having the least reliable frequency estimate.
//
void Channelizer::setAmplitudeWeighting(double x) { _ampWeighting = x; }
...
//! implemented here.
//
double Channelizer::stretchFactor(void) const { return _stretchFactor; }
...
//!             (that is, if fn = n*f1).
//
double Channelizer::computeStretchFactor(double fm, int m, double fn, int n) {
  if (fm <= 0. || fn <= 0.) {
...
//
double Channelizer::computeStretchFactor(double f1, double fn, double n) {
...
//!         frequency and time
//
double Channelizer::computeFractionalChannelNumber(double time,
...
//!         frequency and time
//
int Channelizer::computeChannelNumber(double time, double frequency) const {
...
//
double Channelizer::channelFrequencyAt(double time, int channel) const {
...
// ---------------------------------------------------------------------------
//!	Label a Partial with the number of the frequency channel corresponding
//! to 	the average frequency over all the Partial's Breakpoints.
//!
//!	\param partial is the Partial to label.
//
void Channelizer::channelize(Partial &partial) const {
  using std::pow;
...
//! \throw  InvalidArgument if refChanLabel is not positive.
//
void Channelizer::channelize(PartialList &partials, const Envelope &refChanFreq,
                             int refChanLabel) {
