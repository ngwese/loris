...
//!	at regular time intervals. Resampling makes the envelope data more
//! suitable 	for exchange (as SDIF data, for example) with other applications
//! that 	cannot process raw (continuously-distributed) reassigned data.
//! Resampling 	will often greatly reduce the size of the data (by greatly
//! reducing the 	number of Breakpoints in the Partials) without adversely
//! affecting the 	quality of the reconstruction.
//
class Resampler {
  //	--- public interface ---
public:
  ...
  void operator()(PartialList &plist) const { resample(plist); }
  ...
  // Cannot remove empties without access to the container!
      ...
      //! Resampler's stored quanitization interval. The Breakpoint times in
      //! resampled Partials will comprise a contiguous sequence of all integer
      //! multiples of the sampling interval, starting and ending with the
nearest
      //! multiples to the ends of the Partial. If phase correct resampling is
      //! specified (the default)≤ frequencies and phases are corrected to be in
  //! agreement and to match as nearly as possible the resampled phases.
  ...
  //! Resampling is performed in-place.
      //!
      //!	\param begin is the beginning of the range of Partials to
resample
      //!	\param end is (one-past) the end of the range of Partials to
resample
      //!
...
const #endif
  ...
  */
  ...
  //!         times in resampled Partials onto parameter sampling
  //!         instants in the original Partials.
  void resample(PartialList &plist, const LinearEnvelope &timingEnv) const;
...
#else
 inline
      void quantize( PartialList::iterator begin, PartialList::iterator end  )
const #endif
  {
      while ( begin != end )
      {
