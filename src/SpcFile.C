...
SpcFile::SpcFile(double midiNoteNum)
    : notenum_(midiNoteNum), rate_(DefaultRate) {
...
//
void SpcFile::write(const std::string &filename, double endApproachTime) {
...
//  corresponding to Partials that decay or release normally.
//
void SpcFile::writeSinusoidal(const std::string &filename,
...
//
void SpcFile::write(const std::string &filename, bool enhanced,
                    double endApproachTime) {
...
//  for this SpcFile.
//
SpcFile::markers_type &SpcFile::markers(void) { return markers_; }

const SpcFile::markers_type &SpcFile::markers(void) const { return markers_; }
...
//
double SpcFile::midiNoteNumber(void) const { return notenum_; }
...
//
const SpcFile::partials_type &SpcFile::partials(void) const {
  return partials_;
}
...
//
double SpcFile::sampleRate(void) const { return rate_; }
...
//  allowable maximum.
//
void SpcFile::addPartial(const Partial &p) { addPartial(p, p.label()); }
...
//  allowable maximum.
//
void SpcFile::addPartial(const Partial &p, int label) {
  if (p.label() == 0) {
...
//
void SpcFile::setMidiNoteNumber(double nn) {
  if (nn < 0 || nn > 128) {
    Throw(InvalidArgument,
...
//  proper playback of bandwidth-enhanced Spc data.
//
void SpcFile::setSampleRate(double rate) {
  if (rate <= 0) {
...
// ---------------------------------------------------------------------------
//
void SpcFile::growPartials(partials_type::size_type sz) {
...
struct SpcExportInfo {
  ...
  double
      endApproachTime; //  in seconds, this indicates how long before the end of
                       //  the sound the amplitude, frequency, and bandwidth
                       //  values are to be modified to make a gradual
                       //  transition to the spectral content at the end, 0.0
                       //  indicates no such modifications are to be done
  int numPartials;     //  number of partials in spc file
  int fileNumPartials; //  the actual number of partials plus padding to make a
                       //  2**n value
  ...
  double ampEpsilon; //  small amplitude value (related to lsb value in spc file
                     //  log-amp)
};
...
static void afbp(const Partial &p, double time, double phaseRefTime,
                 double magMult, double freqMult, double &amp, double &freq,
                 double &bw, double &phase) {
...
//
static void pack(double amp, double freq, double bw, double phase, Byte *lbytes,
                 Byte *rbytes) {
  ...
  for (double tim = spcEI.startTime; tim <= spcEI.endTime; tim += spcEI.hop) {
    ...
    for (unsigned int label = 1; label <= spcEI.fileNumPartials; ++label) {
...
#endif
      {
        ...
        double magMult = 0.0;
        afbp(refPar, tim, phaseRefTime, magMult, freqMult, amp, freq, bw,
             phase);
      } else {
        //  find the reference time for the phase
        ...
        //  find amplitude, frequency, bandwidth, phase value
        afbp(partials[label - 1], tim, phaseRefTime, 1, 1, amp, freq, bw,
             phase);
      }
...
//
void configureSosMarkerCk(MarkerCk &ck, const std::vector<Marker> &markers) {
...
// ---------------------------------------------------------------------------
//
std::ostream &writeSosEnvelopesChunk(std::ostream &s,
                                     const SosEnvelopesCk &ck) {
  //  write it out:
  try {
...
// ---------------------------------------------------------------------------
static void configureExportStruct(const SpcFile::partials_type &plist,
                                  double midipitch, double endApproachTime,
                                  int enhanced) {
...
//
static void processEnhancedPoint(Byte *leftbytes, Byte *rightbytes,
                                 const double frameTime, Partial &par) {
...
//
static void processSineOnlyPoint(Byte *bytes, const double frameTime,
                                 Partial &par) {
  //  represent bytes as 24 bit integers:
  const int BytesPerSample = 3;
...
// ---------------------------------------------------------------------------
//
void SpcFile::readSpcData(const std::string &filename) {
  ContainerCk containerChunk;
  CommonCk commonChunk;
  SoundDataCk soundDataChunk;
  InstrumentCk instrumentChunk;
  ...
  try {
    ...
    while (readChunkHeader(s, h)) {
      ...
      case CommonId:
        ...
        if (commonChunk.bitsPerSample != 8 && commonChunk.bitsPerSample != 16 &&
    ...
    if (!commonChunk.header.id || !soundDataChunk.header.id) {
      Throw(FileIOException, "Reached end of file before finding both a Common "
                             "chunk and a Sound Data chunk.");
    }
    if (soseChunk.signature != SosEnvelopesId) {
      Throw(FileIOException,
            "Reached end of file before finding a Spc Envelope Data chunk, "
            "this must not be a Spc file.");
    }
  } catch (Exception &ex) {
  ...
  if (markerChunk.header.id) {
    for (int j = 0; j < markerChunk.numMarkers; ++j) {
      ...
      double markerTime = m.position * hop /
                          (fileNumPartials(numPartials) * (enhanced ? 2 : 1));
