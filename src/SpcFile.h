...
public:
  ...
  SpcFile(PartialList::const_iterator begin_partials,
          PartialList::const_iterator end_partials, double midiNoteNum = 60);
#endif
  ...
  //!	release or decay, but rather end in a static spectrum corresponding to
  ...
  //! frequency, and bandwidth 	values are to be modified to make a gradual
  //! transition to the static spectrum.
  //!
  ...
  //! \param	endApproachTime the duration in seconds of the gradual
  //! transition
...
private:
  ...
  static const int
      MinNumPartials; //!	the minimum number of Partials to export (32)
  ...
  static const double
      DefaultRate; //!	the default Spc export sample rate in Hz (44kHz)
...
SpcFile::SpcFile(Iter begin_partials, Iter end_partials, double midiNoteNum)
    :
#else
inline SpcFile::SpcFile(PartialList::const_iterator begin_partials,
                        PartialList::const_iterator end_partials,
                        double midiNoteNum)
    :
#endif
...
#else
inline void SpcFile::addPartials(PartialList::const_iterator begin_partials,
                                 PartialList::const_iterator end_partials)
#endif
{
