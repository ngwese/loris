...
class Channelizer {
  ...
  std::unique_ptr<Envelope>
      _refChannelFreq; //! the reference frequency envelope
...
public:
...
#else
  inline void operator()(PartialList::iterator begin,
  ...
  //!         refChanFreq tracks the second harmonic, etc.).
  //! \throw  InvalidArgument if refChanLabel is not positive.
  static void channelize(PartialList &partials, const Envelope &refChanFreq,
                         int refChanLabel);
...
#if !defined(NO_TEMPLATE_MEMBERS)
  template <typename Iter>
  static void channelize(Iter begin, Iter end, const Envelope &refChanFreq,
                         int refChanLabel);
#else
  static inline void channelize(PartialList::iterator begin,
                                PartialList::iterator end,
...
#else
inline void Channelizer::channelize(PartialList::iterator begin,
                                    PartialList::iterator end) const
#endif
{
  while (begin != end) {
...
#if !defined(NO_TEMPLATE_MEMBERS)
template <typename Iter>
void Channelizer::channelize(Iter begin, Iter end, const Envelope &refChanFreq,
                             int refChanLabel)
#else
inline void
