...
//
template <class Arg> void setBandwidth(Partial &p, const Arg &arg) {
...
//!			describing the time-varying pitch factor in cents (1/100
//!of
//! a
//!         halfstep).
//
...
public:
  ...
  Cropper(double t1, double t2)
      : minTime(std::min(t1, t2)), maxTime(std::max(t1, t2)) {}
...
//!         should be cropped.
//
inline void crop(Partial &p, double t1, double t2) {
...
//
inline void shiftTime(Partial &p, double offset) {
...
//! \param	offset is a constant offset in seconds.
//
template <class Iter> void shiftTime(Iter b, Iter e, double offset) {
  TimeShifter shifter(offset);
  while (b != e) {
    shifter(*b++);
  }
...
//! \param t    The time before which phases should be adjusted.
//
template <class Iter> void fixPhaseBefore(Iter b, Iter e, double t) {
  while (b != e) {
    fixPhaseBefore(*b, t);
    ++b;
  }
...
//! \param t    The time after which phases should be adjusted.
//
template <class Iter> void fixPhaseAfter(Iter b, Iter e, double t) {
  while (b != e) {
    fixPhaseAfter(*b, t);
    ++b;
  }
...
//! \param t    The time at which phases should be made correct.
//
template <class Iter> void fixPhaseAt(Iter b, Iter e, double t) {
  while (b != e) {
    fixPhaseAt(*b, t);
    ++b;
  }
...
public:
  ...
  //! Function call operator: evaluate a Partial.
  bool operator()(const Partial &p) const { return peakAmplitude(p) < thresh; }
