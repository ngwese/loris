...
class Morpher {
  ...
  double _minBreakpointGapSec; //! the minimum time gap between two Breakpoints
                               ...
                               //! Breakpoints arbitrarily close together in
                               //! time, and this makes morphs huge. Raising
                               //! this threshold limits the Breakpoint density
                               //! in the morphed Partials. Default is 1/10 ms.
...
public:
  ...
  //
  Breakpoint morphBreakpoints(Breakpoint srcBkpt, Breakpoint tgtBkpt,
                              double time) const;
  ...
  //!         to evaluate the morphing functions and tgtPartial).
  //! \return the morphed Breakpoint
  Breakpoint morphSrcBreakpoint(const Breakpoint &bp, const Partial &tgtPartial,
                                double time) const;
  ...
  //!         to evaluate the morphing functions and tgtPartial).
  //! \return the morphed Breakpoint
  Breakpoint morphTgtBreakpoint(const Breakpoint &bp, const Partial &srcPartial,
                                double time) const;
...
private:
  ...
  Breakpoint interpolateParameters(const Breakpoint &srcBkpt,
  ...
  double interpolateFrequency(double srcFreq, double tgtFreq,
                              double alpha) const;
