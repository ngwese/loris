...
//
static void merge(Partial::const_iterator beg, Partial::const_iterator end,
...
//	falls in a zero-amplitude region of plong, then pshort should
// contribute, 	AND its onset should be retained (!!! This is the weird
// part!!!).
...
//  is returned.
//
Partial Distiller::distillOne(PartialList &partials) {
  /*
      debugger << "Distiller found " << partials.size()
                       << " Partials labeled " << partials.front().label() <<
     endl;
  */
  Partial newp;
