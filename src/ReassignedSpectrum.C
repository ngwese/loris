...
//!         samples
//
void ReassignedSpectrum::transform(const double *sampsBegin,
...
//
ReassignedSpectrum::size_type ReassignedSpectrum::size(void) const {
...
//
const std::vector<double> &ReassignedSpectrum::window(void) const {
  return mWindow;
}
...
static std::complex<double> circEvenPartAt(const TransformData &td, long idx) {
...
//
template <class TransformData>
static std::complex<double> circOddPartAt(const TransformData &td, long idx) {
  const long N = td.size();
  while (idx < 0) {
    idx += N;
  }
...
//	a square wave.
//
double ReassignedSpectrum::frequencyCorrection(long idx) const {
...
//
double ReassignedSpectrum::timeCorrection(long idx) const {
  std::complex<double> X_h = circEvenPartAt(mMagnitudeTransform, idx);
...
//!         transform
//
double ReassignedSpectrum::reassignedFrequency(long idx) const {
...
//!         transform
//
double ReassignedSpectrum::reassignedTime(long idx) const {
...
//!         transform
//
double ReassignedSpectrum::reassignedMagnitude(long idx) const {
#if !defined(USE_PARABOLIC_INTERPOLATION)
...
//!         transform
//
double ReassignedSpectrum::reassignedPhase(long idx) const {
...
//!         transform
//
double ReassignedSpectrum::convergence(long idx) const {
...
//
std::complex<double> ReassignedSpectrum::operator[](unsigned long idx) const {
...
//
void ReassignedSpectrum::buildReassignmentWindows(
    const std::vector<double> &window) {
