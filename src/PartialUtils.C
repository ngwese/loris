...
// ---------------------------------------------------------------------------
PartialMutator::PartialMutator(double x) : env(new BreakpointEnvelope(x)) {}

// ---------------------------------------------------------------------------
//	PartialMutator constructor from envelope
// ---------------------------------------------------------------------------
PartialMutator::PartialMutator(const Envelope &e) : env(e.clone()) {}
...
// ---------------------------------------------------------------------------
PartialMutator::~PartialMutator(void) { delete env; }

// ---------------------------------------------------------------------------
//	PartialMutator assignment operator
// ---------------------------------------------------------------------------
PartialMutator &PartialMutator::operator=(const PartialMutator &rhs) {
  if (this != &rhs) {
    delete env;
    env = rhs.env->clone();
  }
...
//
void AmplitudeScaler::operator()(Partial &p) const {
...
//
void BandwidthScaler::operator()(Partial &p) const {
  for (Partial::iterator pos = p.begin(); pos != p.end(); ++pos) {
...
//
void BandwidthSetter::operator()(Partial &p) const {
  for (Partial::iterator pos = p.begin(); pos != p.end(); ++pos) {
...
//
void FrequencyScaler::operator()(Partial &p) const {
  for (Partial::iterator pos = p.begin(); pos != p.end(); ++pos) {
...
//	scale value.
//
void NoiseRatioScaler::operator()(Partial &p) const {
  for (Partial::iterator pos = p.begin(); pos != p.end(); ++pos) {
...
//
void PitchShifter::operator()(Partial &p) const {
  for (Partial::iterator pos = p.begin(); pos != p.end(); ++pos) {
...
//
void Cropper::operator()(Partial &p) const {
...
//
void TimeShifter::operator()(Partial &p) const {
