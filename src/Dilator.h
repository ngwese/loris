...
public:
...
#else
  inline Dilator(const double *ibegin, const double *iend,
                 const double *tbegin);
#endif
  ...
  //!
  //!	\param dilate_begin is the beginning of a sequence of Partials to
  //! dilate. 	\param dilate_end is (one-past) the end of a sequence of
  //! Partials to dilate.
  //!
  //!	If compiled with NO_TEMPLATE_MEMBERS defined, this member accepts
...
#else
  inline void dilate(PartialList::iterator dilate_begin,
...
#else
  inline void operator()(PartialList::iterator dilate_begin,
                         PartialList::iterator dilate_end) const;
#endif

  //!	Return the dilated time value corresponding to the specified initial
  //! time.
  //!
  ...
  //!	\param   dilate_begin is the beginning of a sequence of Partials to
  //! dilate. 	\param   dilate_end is (one-past) the end of a sequence of
  //! Partials to dilate. 	\param 	ibegin is the beginning of a sequence of
  //! initial, or source, 	         time points. 	\param 	iend is
  //! (one-past) the end of a sequence of initial, or 	         source, time
  //! points. 	\param 	tbegin is the beginning of a sequence of target time
  //! points; 	         this sequence must be as long as the sequence of
  //! initial time 	         point described by ibegin and iend.
  //!
  //!	If compiled with NO_TEMPLATE_MEMBERS defined, this member accepts
  //!	only PartialList::const_iterator arguments. Otherwise, this member
  //!   also works for sequences of Markers.
  ...
  static void dilate(PartialsIter dilate_begin, PartialsIter dilate_end,
                     TimeIter1 ibegin, TimeIter1 iend, TimeIter2 tbegin);
#else
  static inline void dilate(PartialList::iterator dilate_begin,
...
#else
inline Dilator::Dilator(const double *ibegin, const double *iend,
                        const double *tbegin)
#endif
{
  while (ibegin != iend) {
...
#else
inline void Dilator::dilate(PartialList::iterator dilate_begin,
                            PartialList::iterator dilate_end) const
#endif
{
...
#else
inline void Dilator::operator()(PartialList::iterator dilate_begin,
                                PartialList::iterator dilate_end) const
#endif
{
...
//!	\sa Dilator::dilate( Partial & p ) const
//
inline void Dilator::operator()(Partial &p) const { dilate(p); }
...
//!	\sa Dilator::dilate( Marker & m ) const
//
inline void Dilator::operator()(Marker &m) const { dilate(m); }
...
//! (one-past) the end of a sequence of initial, or 	         source, time
//! points. 	\param 	tbegin is the beginning of a sequence of target time
//! points; 	         this sequence must be as long as the sequence of initial time 	         point described by ibegin and iend.
//!
//!	If compiled with NO_TEMPLATE_MEMBERS defined, this member accepts
//!	only PartialList::const_iterator arguments. Otherwise, this member
//!   also works for sequences of Markers.
