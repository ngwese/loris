...
//
void KaiserWindow::buildWindow(vector<double> &win, double shape) {
...
//!             the sidelobe rejection level.
//
void KaiserWindow::buildTimeDerivativeWindow(vector<double> &win,
                                             double shape) {
...
//
double KaiserWindow::computeShape(double atten) {
  if (atten < 0.) {
  ...
  if (atten > 60.0) {
    ...
    alpha =
        0.76609L * (pow((atten - 13.26), 0.4)) + 0.09834L * (atten - 13.26L);
  } else {
...
//
unsigned long KaiserWindow::computeLength(double width, double alpha) {
