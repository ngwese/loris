...
public:
  ...
  explicit Distiller(double partialFadeTime = Distiller::DefaultFadeTimeMs /
                                              1000.0,
                     double partialSilentTime = Distiller::DefaultSilentTimeMs /
                                                1000.0);
  ...
  //!
  //! Return an iterator refering to the position of the first unlabeled
  //! Partial, or the end of the distilled collection if there are no unlabeled
  //! Partials. Since distillation is in-place, the Partials collection may be
  //! smaller (fewer Partials) after distillation, and any iterators on the
...
#else
  inline PartialList::iterator distill(PartialList &partials);
#endif
...
template <>
inline PartialList::iterator Distiller::distill(PartialList &partials) {
...
#else
inline PartialList::iterator Distiller::operator()(PartialList &partials)
#endif
{
