...
static bool order_by_time(const Partial_value_type &x,
                          const Partial_value_type &y) {
...
Partial::Partial(const_iterator beg, const_iterator end)
    : _breakpoints(beg._iter, end._iter), _label(0) {
  //	++DebugCounter;
}
...
Partial::Partial(const Partial &other)
    : _breakpoints(other._breakpoints), _label(other._label) {
  //	++DebugCounter;
}
...
//!	Breakpoint in this Partial's envelope.
//
Partial::iterator Partial::begin(void) { return _breakpoints.begin(); }
...
//
Partial::const_iterator Partial::end(void) const { return _breakpoints.end(); }
...
//!	container) does not refer to a valid Breakpoint.
//
Partial::iterator Partial::end(void) { return _breakpoints.end(); }
...
//!	erased range.
//
Partial::iterator Partial::erase(Partial::iterator beg, Partial::iterator end) {
...
//
Partial::const_iterator Partial::findAfter(double time) const {
...
//
Partial::iterator Partial::findAfter(double time) {
#if defined(USE_VECTOR)
  //	see note above
  Partial_value_type dummy(time, Breakpoint());
  return std::upper_bound(_breakpoints.begin(), _breakpoints.end(), dummy,
...
//
Partial::iterator Partial::insert(double time, const Breakpoint &bp) {
#if defined(USE_VECTOR)
  //	see note above
...
//
Partial::size_type Partial::numBreakpoints(void) const {
...
//
Partial::size_type Partial::size(void) const { return _breakpoints.size(); }
...
//
Partial::label_type Partial::label(void) const { return _label; }
...
//!	Breakpoints.
//
const Breakpoint &Partial::first(void) const {
  if (size() == 0) {
    Throw(InvalidPartial,
          "Tried find first Breakpoint in a Partial with no Breakpoints.");
  }
...
//!	Breakpoints.
//
const Breakpoint &Partial::last(void) const {
  if (size() == 0) {
    Throw(InvalidPartial,
          "Tried find last Breakpoint in a Partial with no Breakpoints.");
  }
...
//!	are no Breakpoints.
//
double Partial::initialPhase(void) const {
  if (numBreakpoints() == 0) {
    Throw(InvalidPartial,
...
//!	are no Breakpoints.
//
double Partial::startTime(void) const {
  if (numBreakpoints() == 0) {
    Throw(InvalidPartial,
...
//!	are no Breakpoints.
//
double Partial::endTime(void) const {
  if (numBreakpoints() == 0) {
    Throw(InvalidPartial,
...
//
void Partial::absorb(const Partial &other) {
...
//
void Partial::setLabel(label_type l) { _label = l; }
...
//!	are no Breakpoints.
//
double Partial::duration(void) const {
  if (numBreakpoints() == 0) {
    return 0.;
  }
...
//
Partial::iterator Partial::erase(iterator pos) {
  if (pos != end()) {
    iterator b = pos;
    iterator e = ++pos;
    pos = erase(b, e);
...
void Partial::fadeOut(double fadeTime) {
  ...
  if (0 != bp.amplitude()) {
    insert(endTime() + fadeTime, BreakpointUtils::makeNullAfter(bp, fadeTime));
  }
}
...
//
Partial Partial::split(iterator pos) {
  Partial res(pos, end());
  erase(pos, end());
  return res;
}
...
//!	are no Breakpoints.
//
Partial::const_iterator Partial::findNearest(double time) const {
...
//!	are no Breakpoints.
//
Partial::iterator Partial::findNearest(double time) {
  //	if there are no Breakpoints, return end:
  if (numBreakpoints() == 0) {
    return end();
  }
...
//
double Partial::frequencyAt(double time) const {
...
//
double Partial::amplitudeAt(double time, double fadeTime) const {
...
//!	Breakpoints.
//
double Partial::phaseAt(double time) const {
  Breakpoint bp = parametersAt(time);
  return bp.phase();
}
...
//
double Partial::bandwidthAt(double time) const {
  Breakpoint bp = parametersAt(time);
  return bp.bandwidth();
}
...
//!	Breakpoints.
//
Breakpoint Partial::parametersAt(double time, double fadeTime) const {
  if (numBreakpoints() == 0) {
    Throw(InvalidPartial,
