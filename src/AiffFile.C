...
AiffFile::AiffFile(const std::string &filename)
    : notenum_(60), rate_(1), // rate will be overwritten on import
      numchans_(1) {
...
AiffFile::AiffFile(double samplerate, size_type numFrames /* = 0 */,
    ...
    : notenum_(60), rate_(samplerate), numchans_(numChannels),
...
AiffFile::AiffFile(const double *buffer, size_type bufferlength,
                   double samplerate)
    : notenum_(60), rate_(samplerate), numchans_(1) {
...
AiffFile::AiffFile(const double *buffer_left, const double *buffer_right,
    ...
    : notenum_(60), rate_(samplerate), numchans_(2) {
...
AiffFile::AiffFile(const AiffFile &other)
    : notenum_(other.notenum_), rate_(other.rate_), numchans_(other.numchans_),
      markers_(other.markers_), samples_(other.samples_) {}
...
//
AiffFile &AiffFile::operator=(const AiffFile &rhs) {
  if (&rhs != this) {
...
//!	is assumed.
//
void AiffFile::write(const std::string &filename, unsigned int bps) {
...
//!	for this AiffFile.
//
AiffFile::markers_type &AiffFile::markers(void) { return markers_; }

//!	Return a const reference to the Marker (see Marker.h) container
//!	for this AiffFile.
//
const AiffFile::markers_type &AiffFile::markers(void) const { return markers_; }
...
//
double AiffFile::midiNoteNumber(void) const { return notenum_; }
...
//
unsigned int AiffFile::numChannels(void) const { return numchans_; }
...
//
AiffFile::size_type AiffFile::numFrames(void) const { return samples_.size(); }
...
//!	AiffFile.
//
double AiffFile::sampleRate(void) const { return rate_; }
...
//
AiffFile::samples_type &AiffFile::samples(void) { return samples_; }

//!	Return a const reference (or const reference) to the vector containing
//!	the floating-point sample data for this AiffFile.
//
const AiffFile::samples_type &AiffFile::samples(void) const { return samples_; }
...
//
void AiffFile::addPartial(const Loris::Partial &p, double fadeTime) {
...
//
void AiffFile::setMidiNoteNumber(double nn) {
  if (nn < 0 || nn > 128) {
...
//
Synthesizer AiffFile::configureSynthesizer(double fadeTime) {
...
//
void AiffFile::readAiffData(const std::string &filename) {
  ContainerCk containerChunk;
  CommonCk commonChunk;
  SoundDataCk soundDataChunk;
  InstrumentCk instrumentChunk;
