...
F0Estimate::F0Estimate(const vector<double> &amps, const vector<double> &freqs,
  ...
  if (!eval_freqs.empty()) {
    ...
    //                 << " Hz, Q: "
    //                 << evaluate_Q( amps, freqs, m_frequency, normalization )
    //                 << endl;
    //
...
static double evaluate_Q(const vector<double> &amps,
                         const vector<double> &freqs, double eval_freq,
                         double norm) {
  double prod = std::inner_product(amps.begin(), amps.end(), freqs.begin(), 0.,
                                   std::plus<double>(), Qterm(eval_freq));
...
static double secant_method(const vector<double> &amps,
  ...
  do {
    ...
    fxnm1 = fxn;
      ...
      IsFiniteNumber(deltax) && ++iters < MaxIters);
