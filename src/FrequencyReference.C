...
// ---------------------------------------------------------------------------
//
FrequencyReference *FrequencyReference::clone(void) const {
...
// ---------------------------------------------------------------------------
//
double FrequencyReference::valueAt(double x) const { return _env->valueAt(x); }
...
//
LinearEnvelope FrequencyReference::envelope(void) const { return *_env; }
...
struct IsInFrequencyRange {
  ...
  IsInFrequencyRange(double min, double max) : minFreq(min), maxFreq(max) {
    //	sanity:
    if (maxFreq < minFreq)
      std::swap(minFreq, maxFreq);
  }
