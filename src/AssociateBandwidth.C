...
// ---------------------------------------------------------------------------
//	Association regions are centered on all integer bin frequencies,
// regionWidth 	is the total width (in Hz) of the overlapping bandwidth
// association regions, 	the region centers are spaced at half this
// width.
//
...
// ---------------------------------------------------------------------------
//
static void distribute(double fractionalBin, double x,
...
//
double AssociateBandwidth::computeNoiseEnergy(double freq, double amp) {
...
//
void AssociateBandwidth::accumulateSinusoid(double freq, double amp) {
...
//
void AssociateBandwidth::accumulateNoise(double freq, double amp) {
...
//
void AssociateBandwidth::associate(SpectralPeak &pk) {
  pk.setBandwidth(0);
...
//
void AssociateBandwidth::reset(void) {
...
// peaks 	or ridges. The range [begin, rejected) spans the Peaks selected to
// form 	Partials. The range [rejected, end) spans the Peaks that were found in
//	the reassigned spectrum, but rejected as too weak or too close (in
//	frequency) to another stronger Peak.
//
void AssociateBandwidth::associateBandwidth(
