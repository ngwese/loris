...
PartialBuilder::PartialBuilder(double drift)
    : mFreqWarping(new BreakpointEnvelope(1.0)), mFreqDrift(drift) {}
...
PartialBuilder::PartialBuilder(double drift, const Envelope &env)
    : mFreqWarping(env.clone()), mFreqDrift(drift) {}
...
//
inline double PartialBuilder::warped_freq_distance(const Partial &partial,
...
bool PartialBuilder::better_match(const Partial &part1, const Partial &part2,
                                  const SpectralPeak &pk) {
...
//
void PartialBuilder::buildPartials(Peaks &peaks, double frameTime) {
  ...
  for (Peaks::iterator bpIter = peaks.begin(); bpIter != peaks.end();
    ...
    /*
            if ( nextEligible != mEligiblePartials.end() )
            {
                    debugger << matchFrequency << "( " << end_frequency(
       **eligible )
                                     << ", " << end_frequency( **nextEligible )
       << ")" << endl;
            }
    */
#endif
    ...
    if (eligible != mEligiblePartials.end()) {
      bool matchIsGood = mFreqDrift > std::fabs(end_frequency(**eligible) -
                                                bpIter->frequency());
      if (matchIsGood) {
...
//  set of Partials.
//
PartialList PartialBuilder::finishBuilding(void) {
