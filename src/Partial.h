...
//!	Partial::const_itertator yields a reference to a Breakpoint.
//! Additionally, 	these iterator classes have breakpoint() and time()
//! members, returning 	the Breakpoint (by reference) at the current iterator
//! position and the 	time (by value) corresponding to that Breakpoint.
//!
...
public:
  ...
  //!			iterator.
  Breakpoint &breakpoint(void) const { return _iter->second; }
...
private:
  ...
  Partial_Iterator(const BaseIterator &it) : _iter(it) {}
...
public:
  ...
  //!			a read-only copy.
  Partial_ConstIterator(const Partial_Iterator &other) : _iter(other._iter) {}
...
private:
  //	construction by GenericBreakpointContainer from a BaseIterator:
  Partial_ConstIterator(BaseIterator it) : _iter(it) {}
