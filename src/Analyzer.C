...
//  helpers, used below
// ---------------------------------------------------------------------------
static double accumPeakSquaredAmps(double init, const SpectralPeak &pk) {
  return init + (pk.amplitude() * pk.amplitude());
}

template <class Pair> static double compare2nd(const Pair &p1, const Pair &p2) {
...
public:
  FundamentalBuilder(double fmin, double fmax, double threshDb = -60,
      ...
      : mFminEnv(new LinearEnvelope(fmin)), mFmaxEnv(new LinearEnvelope(fmax)),
        mAmpThresh(std::pow(10., 0.05 * (threshDb))), mFreqThresh(threshHz),
        mMinConfidence(0.9) {}

  FundamentalBuilder(const Envelope &fmin, const Envelope &fmax,
                     double threshDb = -60, double threshHz = 8000)
      : mFminEnv(fmin.clone()), mFmaxEnv(fmax.clone()),
        mAmpThresh(std::pow(10., 0.05 * (threshDb))), mFreqThresh(threshHz),
        mMinConfidence(0.9) {}

  FundamentalBuilder(const FundamentalBuilder &rhs)
      : mFminEnv(rhs.mFminEnv->clone()), mFmaxEnv(rhs.mFmaxEnv->clone()),
        mAmpThresh(rhs.mAmpThresh), mFreqThresh(rhs.mFreqThresh),
        mMinConfidence(rhs.mMinConfidence) {}

  FundamentalBuilder *clone(void) const {
...
void FundamentalBuilder::build(const Peaks &peaks, double frameTime) {
  ...
  for (Peaks::const_iterator spkpos = peaks.begin(); spkpos != peaks.end();
       ++spkpos) {
    if (spkpos->amplitude() > mAmpThresh && spkpos->frequency() < mFreqThresh) {
...
Analyzer::Analyzer(const Analyzer &other)
    : m_freqResolutionEnv(other.m_freqResolutionEnv->clone()),
      m_ampFloor(other.m_ampFloor), m_windowWidth(other.m_windowWidth),
      m_freqFloor(other.m_freqFloor), m_freqDrift(other.m_freqDrift),
      m_hopTime(other.m_hopTime), m_cropTime(other.m_cropTime),
...
//! \param resolutionHz is the frequency resolution in Hz.
//
void Analyzer::configure(double resolutionHz) {
  configure(resolutionHz, 2.0 * resolutionHz);
}
...
//
void Analyzer::configure(double resolutionHz, double windowWidthHz) {
...
//! - independent parameters (bw region width and amp floor)
//
void Analyzer::configure(const Envelope &resolutionEnv, double windowWidthHz) {
  //  use specified resolution:
...
//
PartialList Analyzer::analyze(const std::vector<double> &vec, double srate) {
...
//
PartialList Analyzer::analyze(const double *bufBegin, const double *bufEnd,
                              double srate) {
  BreakpointEnvelope reference(1.0);
...
//
double Analyzer::ampFloor(void) const { return m_ampFloor; }
...
//! for this Analyzer.
//
double Analyzer::cropTime(void) const { return m_cropTime; }
...
//
double Analyzer::freqDrift(void) const { return m_freqDrift; }
...
//
double Analyzer::freqFloor(void) const { return m_freqFloor; }
...
//!		   frequency resolution
//
double Analyzer::freqResolution(double time /* = 0.0 */) const {
...
//! Analyzer.
//
double Analyzer::hopTime(void) const { return m_hopTime; }
...
//! in time.
//
double Analyzer::sidelobeLevel(void) const { return m_sidelobeLevel; }
...
//
double Analyzer::windowWidth(void) const { return m_windowWidth; }
...
bool Analyzer::phaseCorrect(void) const { return m_phaseCorrect; }
...
//
void Analyzer::setAmpFloor(double x) {
...
//! \param x is the new value of this parameter.
//
void Analyzer::setCropTime(double x) {
...
//! \param x is the new value of this parameter.
//
void Analyzer::setFreqDrift(double x) {
...
//! \param x is the new value of this parameter.
//
void Analyzer::setFreqFloor(double x) {
...
//! \param x is the new value of this parameter.
//
void Analyzer::setFreqResolution(double x) {
...
//
void Analyzer::setFreqResolution(const Envelope &e) {
...
//! \param x is the new value of this parameter.
//
void Analyzer::setSidelobeLevel(double x) {
...
//! \param x is the new value of this parameter.
//
void Analyzer::setHopTime(double x) {
...
//! \param x is the new value of this parameter.
//
void Analyzer::setWindowWidth(double x) {
...
//! \param  TF is a flag indicating whether or not to construct
//!         phase-corrected Partials
void Analyzer::setPhaseCorrect(bool TF) { m_phaseCorrect = TF; }
...
//
void Analyzer::storeResidueBandwidth(double regionWidth) {
...
//
void Analyzer::storeConvergenceBandwidth(double tolerance) {
  if (1.0 < tolerance) {
...
//
void Analyzer::storeNoBandwidth(void) { m_bwAssocParam = 0; }
...
// ---------------------------------------------------------------------------
bool Analyzer::bandwidthIsResidue(void) const { return m_bwAssocParam > 0.; }
...
// ---------------------------------------------------------------------------
bool Analyzer::bandwidthIsConvergence(void) const {
...
// ---------------------------------------------------------------------------
double Analyzer::bwRegionWidth(void) const {
  if (m_bwAssocParam > 0) {
...
// ---------------------------------------------------------------------------
double Analyzer::bwConvergenceTolerance(void) const {
  if (m_bwAssocParam < 0) {
    return -m_bwAssocParam;
  }
  return 0;
...
//!         Default is 8 kHz.
//
void Analyzer::buildFundamentalEnv(double fmin, double fmax, double threshDb,
                                   double threshHz) {
...
//
const LinearEnvelope &Analyzer::fundamentalEnv(void) const {
...
struct can_mask {
  ...
  //	constructor:
  can_mask(double x, double y) : _fmin(x), _fmax(y) {
    if (x > y)
      std::swap(x, y);
  }
...
struct negative_time {
  ...
  //	constructor:
  negative_time(double t) : _frameTime(t) {}
...
//	thinPeaks (HELPER)
// ---------------------------------------------------------------------------
//	Reject peaks that are too quiet (low amplitude). Peaks that are
// retained, 	but are quiet enough to be in the specified fadeRange should be
// faded. 	Peaks having negative times are also rejected.
//
...
//	there. It _should_ remove the rejected peaks, but for now, those are
// needed 	by the bandwidth association strategy.
//
Peaks::iterator Analyzer::thinPeaks(Peaks &peaks, double frameTime) {
