...
public:
...
#else
  inline Harmonifier(PartialList::iterator b, PartialList::iterator e,
...
#if !defined(NO_TEMPLATE_MEMBERS)
  template <typename Iter>
  Harmonifier(Iter b, Iter e, Partial::label_type refLabel, const Envelope &env,
              double threshold_dB = 0);
#else
  inline Harmonifier(PartialList::iterator b, PartialList::iterator e,
...
#else
  inline void harmonify(PartialList::iterator b, PartialList::iterator e);
#endif
  ...
  static void harmonify(Iter b, Iter e, Partial::label_type refLabel,
                        double threshold_dB = 0);
#else
  static inline void harmonify(PartialList::iterator b, PartialList::iterator e,
...
Harmonifier::Harmonifier(Iter b, Iter e, Partial::label_type refLabel,
    ...
    :
#else
inline Harmonifier::Harmonifier(PartialList::iterator b,
                                PartialList::iterator e,
                                Partial::label_type refLabel,
                                double threshold_dB)
    :
#endif
      _freqFixThresholdDb(threshold_dB), _weight(createDefaultEnvelope()) {
  if (1 > refLabel) {
...
inline Harmonifier::Harmonifier(PartialList::iterator b,
    ...
    :
#endif
      _freqFixThresholdDb(threshold_dB), _weight(env.clone()) {
  if (1 > refLabel) {
    Throw(InvalidArgument, "The reference label must be positive.");
  }
...
#else
inline void Harmonifier::harmonify(PartialList::iterator b,
                                   PartialList::iterator e)
#endif
{
  while (b != e) {
...
#if !defined(NO_TEMPLATE_MEMBERS)
template <typename Iter>
void Harmonifier::harmonify(Iter b, Iter e, Partial::label_type refLabel,
                            double threshold_dB)
#else
inline void
Harmonifier::harmonify(PartialList::iterator b, PartialList::iterator e,
