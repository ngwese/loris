 ...
 */
extern "C" void channelize(PartialList *partials,
...
/*  Collate unlabeled (zero-labeled) Partials into the smallest-possible
    ...
    Partials.
 */
extern "C" void collate(PartialList *partials) {
  try {
    ThrowIfNull((PartialList *)partials);
...
/*	Return a newly-constructed LinearEnvelope using the legacy
    ...
    estimates taken every five milliseconds.
        ...
        LinearEnvelope.
 */
extern "C" LinearEnvelope *createFreqReference(PartialList *partials,
  ...
  try {
    ...
    if (numSamps != 0) {
      ...
      env_ptr.reset(new LinearEnvelope(FrequencyReference(partials->begin(),
                                                          partials->end(),
                                                          minFreq, maxFreq)
                                           .envelope()));
    }
...
/* Return a newly-constructed LinearEnvelope that estimates
   ...
   LinearEnvelope.
 */
extern "C" LinearEnvelope *createF0Estimate(PartialList *partials,
                                            double minFreq, double maxFreq,
                                            double interval) {
  try {
    ThrowIfNull((PartialList *)partials);
    ...
    LinearEnvelope *env_ptr = new LinearEnvelope(
        est.buildEnvelope(partials->begin(), partials->end(), span.first,
                          span.second, interval, minFreq, maxFreq, Confidence));
    return env_ptr;
  } catch (Exception &ex) {
 ...
 */
extern "C" void dilate(PartialList *partials, const double *initial,
                       const double *target, int npts) {
  try {
    ThrowIfNull((PartialList *)partials);
    ...
    Dilator::dilate(partials->begin(), partials->end(), initial, initial + npts,
                    target);
...
/*  Distill labeled (channelized) Partials in a PartialList into a
    ...
    distilled Partials.
 */
extern "C" void distill(PartialList *partials) {
  try {
    ThrowIfNull((PartialList *)partials);
 ...
 */
extern "C" void exportAiff(const char *path, const double *buffer,
                           unsigned int bufferSize, double samplerate,
                           int bitsPerSamp) {
  try {
 ...
 */
extern "C" void exportSdif(const char *path, PartialList *partials) {
  try {
    ThrowIfNull((PartialList *)partials);
...
/*	Export Partials in a PartialList to a Spc file at the specified file
        ...
        parameter is in seconds. A nonzero endApproachTime indicates that the
plist does not include a release, but rather ends in a static spectrum
corresponding to the final breakpoint values of the partials. The
endApproachTime specifies how long before the end of the sound the amplitude,
frequency, and bandwidth values are to be modified to make a gradual transition
to the static spectrum.
 */
extern "C" void exportSpc(const char *path, PartialList *partials,
                          double midiPitch, int enhanced,
  ...
  try {
    ...
    if (countPartials != partials->size()) {
      notifier << "exporting " << countPartials << " of " << partials->size()
...
/*  Apply a reference Partial to fix the frequencies of Breakpoints
    ...
    unmodified.
 */
extern "C" void harmonify(PartialList *partials, long refLabel,
  ...
  try {
    ...
    Harmonifier::harmonify(partials->begin(), partials->end(), refLabel, *env,
                           threshold_dB);
...
/*	Import audio samples stored in an AIFF file at the given file
        ...
        overwritten.
 */
extern "C" unsigned int importAiff(const char *path, double *buffer,
  ...
  try {
    ...
    AiffFile f(path);
    notifier << "read " << f.samples().size() << " frames at " << f.sampleRate()
             << " Hz" << endl;
...
/*	Import Partials from an SDIF file at the given file path (or
        ...
        see the SDIF web site at:
                www.ircam.fr/equipes/analyse-synthese/sdif/
 */
extern "C" void importSdif(const char *path, PartialList *partials) {
  try {
    ThrowIfNull((PartialList *)partials);
 ...
 */
extern "C" void importSpc(const char *path, PartialList *partials) {
  try {
    ...
    // partials->insert( partials->end(), imp.partials().begin(),
    // imp.partials().end() );
...
extern "C" void morpher_setAmplitudeShape(double x) {
  if (x <= 0.) {
    std::string s("Loris exception in morpher_setAmplitudeShape(): ");
    s.append("Invalid Argument: the amplitude morph shaping parameter must be "
             "positive");
    handleException(s.c_str());
  }
  PI_ampMorphShape = x;
 ...
 */
extern "C" void morph(const PartialList *src0, const PartialList *src1,
  ...
  try {
    ...
    notifier << "morphing " << src0->size() << " Partials with " << src1->size()
             << " Partials" << endl;
 ...
 */
extern "C" void resample(PartialList *partials, double interval) {
  try {
    ThrowIfNull((PartialList *)partials);
 ...
 */
extern "C" void shapeSpectrum(PartialList *partials, PartialList *surface,
 ...
 */
extern "C" void sift(PartialList *partials) {
  try {
    ThrowIfNull((PartialList *)partials);
 ...
 */
extern "C" unsigned int synthesize(const PartialList *partials, double *buffer,
                                   unsigned int bufferSize, double srate) {
  unsigned int howMany = 0;
  try {
    ThrowIfNull((PartialList *)partials);
    ThrowIfNull((double *)buffer);

    notifier << "synthesizing " << partials->size() << " Partials at " << srate
             << " Hz" << endl;
