...
Morpher::Morpher(const Envelope &f)
    : _freqFunction(f.clone()), _ampFunction(f.clone()), _bwFunction(f.clone()),
      _freqFixThresholdDb(DefaultFixThreshold), _logMorphShape(DefaultAmpShape),
...
Morpher::Morpher(const Envelope &ff, const Envelope &af, const Envelope &bwf)
    : _freqFunction(ff.clone()), _ampFunction(af.clone()),
      _bwFunction(bwf.clone()), _freqFixThresholdDb(DefaultFixThreshold),
...
Morpher::Morpher(const Morpher &rhs)
    : _freqFunction(rhs._freqFunction->clone()),
      ...
      _bwFunction(rhs._bwFunction->clone()), _srcRefPartial(rhs._srcRefPartial),
...
//!
//! \param  rhs is the Morpher to duplicate
Morpher &Morpher::operator=(const Morpher &rhs) {
  if (&rhs != this) {
...
//
Partial Morpher::morphPartials(Partial src, Partial tgt, int assignLabel) {
...
void Morpher::fixMorphedPhases(Partial &newp) const {
  if (0 != newp.numBreakpoints()) {
...
//
void Morpher::crossfade(PartialList::const_iterator beginSrc,
  ...
  for (it = beginSrc; it != endSrc; ++it) {
    if (it->label() == label && 0 != it->numBreakpoints()) {
      ...
      for (Partial::const_iterator bpPos = it->begin(); bpPos != it->end();
           ++bpPos) {
        //  Don't insert Breakpoints arbitrarily close together,
        //  only insert a new Breakpoint if it is later than
        //  the end of the new Partial by more than the gap time.
...
//
void Morpher::morph(PartialList::const_iterator beginSrc,
                    PartialList::const_iterator endSrc,
                    PartialList::const_iterator beginTgt,
                    PartialList::const_iterator endTgt) {
...
//
Breakpoint Morpher::morphBreakpoints(Breakpoint srcBkpt, Breakpoint tgtBkpt,
                                     double time) const {
...
//!    \return the morphed Breakpoint
//
Breakpoint Morpher::morphSrcBreakpoint(const Breakpoint &srcBkpt,
...
//!    \return the morphed Breakpoint
//
Breakpoint Morpher::morphTgtBreakpoint(const Breakpoint &tgtBkpt,
...
//
Breakpoint Morpher::fadeSrcBreakpoint(Breakpoint bp, double time) const {
...
//! \return the faded Breakpoint
//
Breakpoint Morpher::fadeTgtBreakpoint(Breakpoint bp, double time) const {
  double alpha = _ampFunction->valueAt(time);
...
//
void Morpher::setFrequencyFunction(const Envelope &f) {
...
//
void Morpher::setAmplitudeFunction(const Envelope &f) {
...
//
void Morpher::setBandwidthFunction(const Envelope &f) {
...
//
const Envelope &Morpher::frequencyFunction(void) const {
...
//
const Envelope &Morpher::amplitudeFunction(void) const { return *_ampFunction; }
...
//
const Envelope &Morpher::bandwidthFunction(void) const { return *_bwFunction; }
...
//
const Partial &Morpher::sourceReferencePartial(void) const {
...
//! should be used for the source sequence.
//
Partial &Morpher::sourceReferencePartial(void) { return _srcRefPartial; }
...
//
const Partial &Morpher::targetReferencePartial(void) const {
...
//! should be used for the target sequence.
//
Partial &Morpher::targetReferencePartial(void) { return _tgtRefPartial; }
...
//
void Morpher::setSourceReferencePartial(const Partial &p) {
  if (p.label() == 0) {
...
//
void Morpher::setSourceReferencePartial(const PartialList &partials,
                                        Partial::label_type refLabel) {
  if (refLabel != 0) {
    PartialList::const_iterator pos = std::find_if(
        partials.begin(), partials.end(), PartialUtils::isLabelEqual(refLabel));
    if (pos == partials.end()) {
...
//
void Morpher::setTargetReferencePartial(const Partial &p) {
  if (p.label() == 0) {
...
//!         that should be selected as the reference
//
void Morpher::setTargetReferencePartial(const PartialList &partials,
                                        Partial::label_type refLabel) {
  if (refLabel != 0) {
    PartialList::const_iterator pos = std::find_if(
        partials.begin(), partials.end(), PartialUtils::isLabelEqual(refLabel));
...
double Morpher::amplitudeShape(void) const { return _logMorphShape; }
...
double Morpher::minBreakpointGap(void) const { return _minBreakpointGapSec; }
...
//
PartialList &Morpher::partials(void) { return _partials; }
...
//
const PartialList &Morpher::partials(void) const { return _partials; }
...
//
static inline double interpolateLog(double x, double y, double alpha,
                                    double shape) {
  using std::pow;
...
//  alpha == 0 returns x, alpha == 1 returns y
//
static inline double interpolateLinear(double x, double y, double alpha) {
...
//
inline double Morpher::interpolateAmplitude(double srcAmp, double tgtAmp,
                                            double alpha) const {
  double morphedAmp = 0;
...
//
inline double Morpher::interpolateBandwidth(double srcBw, double tgtBw,
                                            double alpha) const {
  double morphedBw = 0;
...
//
inline double Morpher::interpolateFrequency(double srcFreq, double tgtFreq,
                                            double alpha) const {
...
//
inline double Morpher::interpolatePhase(double srcphase, double tgtphase,
                                        double alpha) const {
...
//
inline Breakpoint Morpher::interpolateParameters(const Breakpoint &srcBkpt,
                                                 const Breakpoint &tgtBkpt,
                                                 double fweight, double aweight,
                                                 double bweight) const {
  Breakpoint morphed;

  // interpolate frequencies:
  morphed.setFrequency(
      interpolateFrequency(srcBkpt.frequency(), tgtBkpt.frequency(), fweight));

  // interpolate LOG amplitudes:
  morphed.setAmplitude(
      interpolateAmplitude(srcBkpt.amplitude(), tgtBkpt.amplitude(), aweight));

  // interpolate bandwidth:
  morphed.setBandwidth(
      interpolateBandwidth(srcBkpt.bandwidth(), tgtBkpt.bandwidth(), bweight));

  // interpolate phase:
  morphed.setPhase(interpolatePhase(srcBkpt.phase(), tgtBkpt.phase(), fweight));
...
//
void Morpher::appendMorphedSrc(Breakpoint srcBkpt, const Partial &tgtPartial,
  ...
  if (fweight < MaxMorphParam || aweight < MaxMorphParam ||
    ...
    adjustFrequency(srcBkpt, _srcRefPartial, newp.label(), _freqFixThresholdDb,
                    time);
    ...
    if (0 == tgtPartial.numBreakpoints()) {
      ...
      // compute interpolated Breakpoint parameters:
      Breakpoint morphed =
          interpolateParameters(srcBkpt, tgtBkpt, fweight, aweight, bweight);
...
//!         Breakpoint is added to this Partial.
//
void Morpher::appendMorphedTgt(Breakpoint tgtBkpt, const Partial &srcPartial,
                               double time, Partial &newp) {
  double fweight = _freqFunction->valueAt(time);
  double aweight = _ampFunction->valueAt(time);
  double bweight = _bwFunction->valueAt(time);
  ...
  if (fweight > MinMorphParam || aweight > MinMorphParam ||
    ...
    // adjust target Breakpoint frequencies according to the reference
    // Partial (if a reference has been specified):
    adjustFrequency(tgtBkpt, _tgtRefPartial, newp.label(), _freqFixThresholdDb,
                    time);
