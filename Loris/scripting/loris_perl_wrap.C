...
XS(_wrap_extract) {
    ...
    const char *_swigerr = _swigmsg;
    {
        ...
        if ((items < 2) || (items > 2)) {
            SWIG_croak("Usage: extract(partials,label);");
        }
        {
            if (SWIG_ConvertPtr(ST(0), (void **) &arg1, SWIGTYPE_p_PartialList,0) < 0) {
                SWIG_croak("Type error in argument 1 of extract. Expected _p_PartialList");
            }
        }
        arg2 = (long) SvIV(ST(1));
        {
            *EXCEPTION_THROWN = '\0';
            result = (PartialList *)extract(arg1,arg2);
...
{"loris_perl::extract", _wrap_extract},
