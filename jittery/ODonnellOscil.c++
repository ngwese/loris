...
Oscillator::Oscillator( void ) :
	...
	coherentJitter( one_pole_filter( 0.9999 ) ),
	incoherentJitter( one_pole_filter( 0.9999 ) ),
	i_frequency( 0 ),
	i_amplitude( 0 ),
	i_bandwidth( 0 ),
	i_jitter( 0 ),
...
//
void 
Oscillator::resetJitter( double jgain, double jcoherence, unsigned long startingSamp )
{
	i_jitter = jgain;
	i_coherence = jcoherence;
	pm_coherent = 0.;
	pm_incoherent = 0.;

	//	reset the coherent jitter modulator, and advance
	//	it to the starting sample for this Partial
	coherentJitter.reset( 1 );
	for ( unsigned long samp = 0; samp < startingSamp; ++samp )
	{
		coherentJitter.next();
	}	
}
...
Oscillator::oscillate( double * begin, double * end,
					   ...
					   double targetJitter, double targetCoherence,
					   int partialNum )
{
	...
	//	clamp coherence:
	if ( targetCoherence > 1. )
	{
		debugger << "clamping coherence at 1." << endl;
		targetCoherence = 1.;
	}
	else if ( targetCoherence < 0. )
	{ 
		debugger << "clamping coherence at 0." << endl;
		targetCoherence = 0.;
	}
	...
	const double dJitter = (targetJitter - i_jitter)  * dTime;
	const double dCoherence = (targetCoherence - i_coherence)  * dTime;
	...
	for ( double * putItHere = begin; putItHere != end; ++putItHere )
	{
		...
		*putItHere += am * i_amplitude * cos( determ_phase + pm_coherent + pm_incoherent );
			
		//	-------- JITTER --------
		//	update coherent and incoherent phase
	...
	i_jitter = targetJitter;
	i_coherence = targetCoherence;
}
